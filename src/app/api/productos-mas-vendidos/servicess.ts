...
export async function getProducts(limit: number, offset: number): Promise<{ products: producto[], total: number }> {
  try {
    ...
    const result = await query("SELECT * FROM   GetProductoMasVist($1, $2, $3, $4, $5, $6, $7)", ['', 'P',null, null, true, 8, offset]);

    // Obtener el número total de productos para calcular las páginas
    const total  = await query("SELECT * FROM GetProductoCount($1, $2, $3, $4, $5, $6)", [null, 'P',null, null, null, true,]);
